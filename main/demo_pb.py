...
def draw_boxes(img, image_name, boxes, scale):
    ...
    img = cv2.resize(img, None, None, fx=1.0 / scale, fy=1.0 /
...
if __name__ == '__main__':
    ...
    with gfile.FastGFile('checkpoints_mlt/frozen_model.pb', 'rb') as f:
    ...
    for im_name in im_names:
        ...
        if cfg.TEST.HAS_RPN:
            ...
            blobs['im_info'] = np.array(
                ...
                dtype=np.float32)
        cls_prob, box_pred = sess.run([output_cls_prob, output_box_pred], feed_dict={
                                      input_img: blobs['data']})
        ...
        boxes = textdetector.detect(
            boxes, scores[:, np.newaxis], img.shape[:2])
